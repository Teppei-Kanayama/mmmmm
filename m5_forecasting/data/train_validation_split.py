...
class TrainValidationSplit(gokart.TaskOnKart):
    ...
    def run(self):
        ...
        x_train, y_train, x_val, y_val = self._run(data, self.validation_days, self.train_to_date)
    ...
    def _run(data: pd.DataFrame, validation_days: int, train_to_date: int) -> Tuple[pd.DataFrame, ...]:
        x_train = data[data['d'] < train_to_date - validation_days]
        y_train = x_train['demand']
        x_val = data[(data['d'] < train_to_date) & (data['d'] >= train_to_date - validation_days)]
        y_val = x_val['demand']
