...
class LoadInputData(gokart.TaskOnKart):
    ...
    def output(self):
        calender = self.make_target('input/calendar.csv', use_unique_id=False)
        sales_train_validation = self.make_target('input/sales_train_validation.csv', use_unique_id=False)
        sample_submission = self.make_target('input/sample_submission_accuracy.csv', use_unique_id=False)
        sell_prices = self.make_target('input/sell_prices.csv', use_unique_id=False)
        return dict(calender=calender, sales_train_validation=sales_train_validation,
