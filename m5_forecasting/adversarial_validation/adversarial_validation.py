...
class AdversarialValidation(gokart.TaskOnKart):
    ...
    def requires(self):
        ...
        return dict(feature=feature_task)

    def run(self):
        feature = self.load_data_frame('feature')
        feature_columns = [feature_column for feature_column in feature.columns if feature_column not in ['id', 'd', 'year', 'week_of_year', 'day']]

        target_term = (1914 - 365, 1914 - 1)
        source_term = (1914 - 365 * 2, 1914 - 365 - 1)

        target_feature = feature[feature['d'].between(*target_term)]
        source_feature = feature[feature['d'].between(*source_term)]
        ...
        params = {"objective": "binary", "metric": "auc", "force_row_wise": True, "learning_rate": 0.075,
        ...
        model = lgb.train(params, train_set, num_boost_round=num_boost_round,
                          early_stopping_rounds=early_stopping_rounds, valid_sets=valid_sets, verbose_eval=100)
