...
class TrainPointwiseModel(gokart.TaskOnKart):
    ...
    def requires(self):
        ...
        sales_data_task = MekeSalesFeature(sales_data_task=PreprocessSales(is_small=self.is_small, to_date=self.train_to_date),
                                           predicted_sales_data_task=DummyPredictTask(),
                                           make_feature_from_date=self.train_from_date,
